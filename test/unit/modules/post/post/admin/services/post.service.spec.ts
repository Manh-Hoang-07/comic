...
describe('PostService', () => {
    ...
    describe('getOne', () => {
        it('should verify group ownership', async () => {
            ...
            jest.spyOn(Object.getPrototypeOf(PostService.prototype), 'getOne').mockResolvedValue(entity);
    ...
    describe('transform', () => {
        it('should map primary_category, categories, and tags', () => {
            const post = {
                ...
                categories: [{ category: { id: 2, name: 'Cat2', slug: 'cat2', extra: 2 } }],
                tags: [{ tag: { id: 3, name: 'Tag3', slug: 'tag3', extra: 3 } }]
            };

            const result = (service as any).transform(post);

            expect(result.primary_category).toEqual({ id: 1, name: 'Cat', slug: 'cat' });
            expect(result.categories).toEqual([{ id: 2, name: 'Cat2', slug: 'cat2' }]);
