...
> implements IUserRoleAssignmentRepository {
    ...
    async findActivePermissionCodes(userId: any, groupId: any | null): Promise<string[]> {
    ...
    async syncRolesInGroup(userId: any, groupId: any, roleIds: any[]): Promise<void> {
        ...
        await this.prisma.$transaction(async (tx) => {
            if (roleIdsPk.length > 0) {
                await tx.userGroup.upsert({
                    where: { user_id_group_id: { user_id: userIdPk, group_id: groupIdPk } },
                    create: { user_id: userIdPk, group_id: groupIdPk, joined_at: new Date() },
                    update: {},
                });
            }
